...
doc = """
Introduction and Quiz to Multimarket Contact experiment Treatment T-MSym
"""
...
def frame_question(contProb, num_matches, time_quiz, time_round, time_intro, expPoints_Red, payoff_matrix1, payoff_matrix2):
    ...
    questions[5]['statement'] = models.StringField(initial="In Period 2, how many points did Other receive in Red Game? (Refer to the History Tables above)")
    ...
    questions[6]['statement'] = models.StringField(initial="In a Period, if you choose Action W and Other chooses Action W in Blue Game, then how many points will you receive in Blue Game? (Refer to the Points Tables above)")
    ...
    questions[8]['statement'] = models.StringField(initial="In Period 3, which Action was chosen by Other in Red Game? (Refer to the History Tables above)")
...
class Constants(BaseConstants):
    ...
    time_welcome = settings.SESSION_CONFIGS[2].get('time_welcome') #Time alloted to first page
    time_quiz = settings.SESSION_CONFIGS[2].get('time_quiz') #Total time alloted to the quiz
    time_round = settings.SESSION_CONFIGS[2].get('time_round') #Total time alloted to a round
    time_intro = settings.SESSION_CONFIGS[2].get('time_intro') #Time alloted to instructions
