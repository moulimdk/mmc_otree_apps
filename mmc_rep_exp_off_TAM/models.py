...
class Constants(BaseConstants):
    name_in_url = 'mmc_rep_exp_off_TAM'
    ...
    expPoints = {}
    expPoints["R"] = settings.SESSION_CONFIGS[7].get('expPoints_Red') # Expected Points matrix 1
    expPoints["B"] = settings.SESSION_CONFIGS[7].get('expPoints_Blue') # Expected Points matrix 2
    
    SG_lengths = settings.SESSION_CONFIGS[7].get('SG_lengths') # Number of periods in supergames
    ...
    time_period = settings.SESSION_CONFIGS[7].get('time_period') # Total time alloted to a period
    time_begin = settings.SESSION_CONFIGS[7].get('time_welcome') # Time alloted in the first page
    time_next = settings.SESSION_CONFIGS[7].get('time_next') # Time to next round
...
class Group(BaseGroup):
    ...
    def period_update(self):
        ...
        p1.myPayoffR = Constants.payoff_matrix1[p1.myChoiceR][p2.myChoiceB]
        p1.myPayoffB = Constants.payoff_matrix2[p1.myChoiceB][p2.myChoiceR]
        p2.myPayoffR = Constants.payoff_matrix1[p2.myChoiceR][p1.myChoiceB]
        p2.myPayoffB = Constants.payoff_matrix2[p2.myChoiceB][p1.myChoiceR]

        p1.myChoiceHistoryR += ","+str(p1.myChoiceR)
        p1.myPayoffHistoryR += ","+str(p1.myPayoffR)
        p1.otherChoiceHistoryR += ","+str(p2.myChoiceB)
        p1.otherPayoffHistoryR += ","+str(p2.myPayoffB)

        p1.myChoiceHistoryB += ","+str(p1.myChoiceB)
        p1.myPayoffHistoryB += ","+str(p1.myPayoffB)
        p1.otherChoiceHistoryB += ","+str(p2.myChoiceR)
        p1.otherPayoffHistoryB += ","+str(p2.myPayoffR)
        ...
        p2.otherChoiceHistoryR += ","+str(p1.myChoiceB)
        p2.otherPayoffHistoryR += ","+str(p1.myPayoffB)

        p2.myChoiceHistoryB += ","+str(p2.myChoiceB)
        p2.myPayoffHistoryB += ","+str(p2.myPayoffB)
        p2.otherChoiceHistoryB += ","+str(p1.myChoiceR)
        p2.otherPayoffHistoryB += ","+str(p1.myPayoffR)
