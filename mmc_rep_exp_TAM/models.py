...
doc = """
Experiment for Treatment Asymmetric Multimarket Contact
"""

class Constants(BaseConstants):
    name_in_url = 'mmc_rep_exp_TAM'
    players_per_group = None
     ## Payoff Tables
    payoff_matrix1=settings.SESSION_CONFIGS[6].get('payoff_1') # Payoff matrix of setting 1
    payoff_matrix2=settings.SESSION_CONFIGS[6].get('payoff_2') # Payoff matrix of setting 2
    expPoints = {}
    expPoints["R"] = settings.SESSION_CONFIGS[6].get('expPoints_Red') # Expected Points matrix 1
    expPoints["B"] = settings.SESSION_CONFIGS[6].get('expPoints_Blue') # Expected Points matrix 2
    
    SG_lengths = settings.SESSION_CONFIGS[6].get('SG_lengths') # Number of periods in supergames
    ...
    dieN = settings.SESSION_CONFIGS[6].get('die_N') #Number on a die
    contProb = settings.SESSION_CONFIGS[6].get('contProb') #Continuation probability in percentages
    endProb = 100 - contProb 
    thres = int(1+(dieN*contProb/100)); #Threshold number on a die

    participation_fee = settings.SESSION_CONFIGS[6].get('participation_fee') # Participation payment
    quiz_fee = settings.SESSION_CONFIGS[6].get('quiz_fee') # Quiz payment

    exRate = int(1/settings.SESSION_CONFIGS[6].get('real_world_currency_per_point')) # Exchange Rate (points to dollar)
    
    time_period = settings.SESSION_CONFIGS[6].get('time_period') # Total time alloted to a period
    time_begin = settings.SESSION_CONFIGS[6].get('time_welcome') # Time alloted in the first page
    time_next = settings.SESSION_CONFIGS[6].get('time_next') # Time to next round
...
class Group(BaseGroup):
    ...
    def period_outcome(self):
        ...
        if (p1.participant.vars["disconnected"] == 0) & (p2.participant.vars["disconnected"] == 0):
            ...
            p1lastPayoffR = Constants.payoff_matrix1[p1lastChoiceR][p2lastChoiceB]
            p1lastPayoffB = Constants.payoff_matrix2[p1lastChoiceB][p2lastChoiceR]
            p2lastPayoffR = Constants.payoff_matrix1[p2lastChoiceR][p1lastChoiceB]
            p2lastPayoffB = Constants.payoff_matrix2[p2lastChoiceB][p1lastChoiceR]
            ...
            p1.otherChoiceHistoryR = p1.in_round((self.round_number - 1)).otherChoiceHistoryR+","+str(p2lastChoiceB)
            p1.otherChoiceHistoryB = p1.in_round((self.round_number - 1)).otherChoiceHistoryB+","+str(p2lastChoiceR)
            p1.otherPayoffHistoryR = p1.in_round((self.round_number - 1)).otherPayoffHistoryR+","+str(p2lastPayoffB)
            p1.otherPayoffHistoryB = p1.in_round((self.round_number - 1)).otherPayoffHistoryB+","+str(p2lastPayoffR)
            ...
            p2.otherChoiceHistoryR = p2.in_round((self.round_number - 1)).otherChoiceHistoryR+","+str(p1lastChoiceB)
            p2.otherChoiceHistoryB = p2.in_round((self.round_number - 1)).otherChoiceHistoryB+","+str(p1lastChoiceR)
            p2.otherPayoffHistoryR = p2.in_round((self.round_number - 1)).otherPayoffHistoryR+","+str(p1lastPayoffB)
            p2.otherPayoffHistoryB = p2.in_round((self.round_number - 1)).otherPayoffHistoryB+","+str(p1lastPayoffR)
    ...
    def end_period_outcome(self):
        ...
        if (p1.participant.vars["disconnected"] == 0) & (p2.participant.vars["disconnected"] == 0):
            ...
            p1.otherChoiceHistoryR += ","+str(p2lastChoiceB)
            p1.otherChoiceHistoryB += ","+str(p2lastChoiceR)
            p1.otherPayoffHistoryR += ","+str(p2lastPayoffB)
            p1.otherPayoffHistoryB += ","+str(p2lastPayoffR)
            ...
            p2.otherChoiceHistoryR += ","+str(p1lastChoiceB)
            p2.otherChoiceHistoryB += ","+str(p1lastChoiceR)
            p2.otherPayoffHistoryR += ","+str(p1lastPayoffB)
            p2.otherPayoffHistoryB += ","+str(p1lastPayoffR)
