...
doc = """
Introduction and Quiz to Multimarket Contact experiment Treatment T-MAsym
"""
...
class Constants(BaseConstants):
    name_in_url = 'mmc_rep_intro_TAM'
    players_per_group = None
    num_rounds = 1
    
    #Properties of session imported from session configurations
    num_matches = settings.SESSION_CONFIGS[6].get('SG_totalNum') #Total Number of Supergames
    ...
    avg_earn = settings.SESSION_CONFIGS[6].get('avg_earn') #Average Earning
    low_earn = settings.SESSION_CONFIGS[6].get('low_earn') #Low Earning
    high_earn = settings.SESSION_CONFIGS[6].get('high_earn') #High Earning
    expPoints_Red = settings.SESSION_CONFIGS[6].get('expPoints_Red') #Expected Points for Red Game
    expPoints_Blue = settings.SESSION_CONFIGS[6].get('expPoints_Blue') #Expected Points for Blue Game

    exRate = int(1/settings.SESSION_CONFIGS[6].get('real_world_currency_per_point')) #Exchange Rate (points to dollar)
    
    minExpTime = settings.SESSION_CONFIGS[6].get('minExpTime') #Min Expected time of the session
    maxExpTime = settings.SESSION_CONFIGS[6].get('maxExpTime') #Max Expected time of the session
    time_welcome = settings.SESSION_CONFIGS[6].get('time_welcome') #Time alloted to first page
    time_quiz = settings.SESSION_CONFIGS[6].get('time_quiz') #Total time alloted to the quiz
    time_round = settings.SESSION_CONFIGS[6].get('time_round') #Total time alloted to a round
    time_intro = settings.SESSION_CONFIGS[6].get('time_intro') #Time alloted to instructions
    
    num_ques = settings.SESSION_CONFIGS[6].get('num_ques') #Number of questions in the quiz
